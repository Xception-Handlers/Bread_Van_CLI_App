...
# IMPORTANT: we import initialize here
from App.controllers.initialize import initialize
...
def create_app(overrides={}):
    ...
    # ---- Blueprints / DB / Admin / JWT / Errors ----
    add_views(app)
    init_db(app)
    jwt = setup_jwt(app)
    setup_admin(app)
    ...
    # Push context so CLI & other code can use current_app / db
    app.app_context().push()

    # ---- Auto-init for real deployments ----
    from App.controllers.initialize import initialize
    ...
    if not is_testing and auto_init:
        try:
            ...
            # Log so you can see it in Render logs, but don’t crash tests
            app.logger.error(f"Auto-init failed: {e}")
